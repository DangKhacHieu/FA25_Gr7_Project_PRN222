    ...
    public class CartService : ICartService
    {
        ...
        public async Task<OperationResult> AddToCartWithCheckAsync(int customerId, int productId, int quantity)
        {
            ...
            if (product == null || product.IsDelete == 1)
                return OperationResult.Fail("❌ Sản phẩm không tồn tại hoặc đã bị xóa.");

            if (product.Quantity_Product <= 0)
                return OperationResult.Fail($"❌ Sản phẩm {product.ProductName} đã hết hàng.");
            ...
            if (totalQuantityAfterAdd > product.Quantity_Product)
                return OperationResult.Fail($"⚠️ Sản phẩm {product.ProductName} chỉ còn {product.Quantity_Product} cái trong kho.");

            await _cartRepository.AddToCartAsync(customerId, productId, quantity);
            return OperationResult.Ok("✅ Đã thêm sản phẩm vào giỏ hàng!");
        }

        public async Task<OperationResult> UpdateCartItemWithCheckAsync(int cartItemId, int quantity)
        {
            var cartItem = await _cartRepository.GetCartItemByIdAsync(cartItemId);
            if (cartItem == null) return OperationResult.Fail("❌ Không tìm thấy sản phẩm trong giỏ hàng.");

            var product = await _productRepository.GetByIdAsync(cartItem.ProductId);
            if (product == null || product.IsDelete == 1)
                return OperationResult.Fail("❌ Sản phẩm này hiện không còn tồn tại.");

            if (quantity > product.Quantity_Product)
                return OperationResult.Fail($"⚠️ Số lượng tối đa có thể mua là {product.Quantity_Product}.");

            await _cartRepository.UpdateCartItemAsync(cartItemId, quantity);
            return OperationResult.Ok("✅ Cập nhật giỏ hàng thành công!");
        }
